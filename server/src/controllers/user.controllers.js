...
const generateAccessAnsRefreshToken = async(userId) => {
    try {
        ...
        throw new error(500, "Something is wrong while generating the token")
    }
}
...
const registerUser = asyncHandler( async (req, res) => {
    ...
    // const coverImageLocalPath = req.files?.coverImage[0]?.path; //if itsnot defined then it will throw eerror
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    return res.status(200).cookie("accessToken", accessToken, options).cookie("accessToken", refreshToken, options).json(
        new ApiResponse(
            200,{
